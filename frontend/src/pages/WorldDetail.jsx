...
const WorldDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-white">
      ...
      className="bg-gradient-to-r from-[#1a1f3c] to-[#0B0F1A] px-4 sm:px-6 md:px-10 py-6 md:py-10">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4">{world.name}</h1>
        <p className="text-sm sm:text-base text-gray-300 max-w-2xl mb-4">{world.description}</p>
        <button 
          onClick={() => {
          ...
          className="px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base bg-[#B19EEF] text-black font-semibold rounded-lg hover:bg-[#9d85e0] transition-colors cursor-pointer"
        >
          Start Learning
        </button>
      </div>
      ...
      {/* MAIN CONTENT */}
      <div className="flex flex-col lg:flex-row px-4 sm:px-6 md:px-10 py-6 md:py-10 gap-6 md:gap-10">
        {/* LEFT – MAP */}
              ...
              <div className="space-y-2">
                {zone.stages.map((stage, i) => {
                  ...
                  return (
                    <div
                      key={stage.id}
                        className={`flex flex-col sm:flex-row justify-between items-start sm:items-center px-3 py-2 rounded-lg transition gap-2 ${
                          isCompleted
                      ...
                      <button
                        ...
                        className="text-xs sm:text-sm px-3 py-1 w-full sm:w-auto text-center bg-[#B19EEF]/20 border border-[#B19EEF]/40 rounded hover:bg-[#B19EEF]/30 transition-colors cursor-pointer"
                      >
        ...
        {/* RIGHT – USER PANEL */}
        <div className="w-full lg:w-[320px] space-y-4 md:space-y-6">
          {/* Course Progress Section */}
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 md:p-6 space-y-4">
          <div className="flex flex-col sm:flex-row gap-3 items-center sm:items-start">
            <img  className="h-16 md:h-20 flex-shrink-0" src="/assets/png&gif/gif/boy.gif" alt="boy" />
            <h3 className="font-semibold text-base md:text-lg text-center sm:text-left">Course Progress</h3>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-black/40 rounded-lg p-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-1">
                  <span className="text-xs sm:text-sm text-gray-400">Levels Completed</span>
                  <span className="text-xl md:text-2xl font-bold text-[#B19EEF]">{completedCount}/{totalStages}</span>
                </div>
                <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-linear-to-r from-[#B19EEF] to-[#9d85e0] transition-all duration-500"
                    style={{ width: `${completionPercentage}%` }}
                  />
                </div>
                <p className="text-xs text-gray-400 mt-2">{completionPercentage.toFixed(0)}% Complete</p>
              </div>

              {/* XP Stats */}
              <div className="bg-black/40 rounded-lg p-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <span className="text-xs sm:text-sm text-gray-400">Total XP Earned</span>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl md:text-2xl font-bold text-[#B19EEF]">{stats?.totalXP || 0}</span>
                  </div>
                </div>
              </div>
            </div>
